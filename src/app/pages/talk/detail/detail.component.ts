...
export class DetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      next: ({ entry, quote }) => {
        ...
        this.metaTag.updateTag({
          name: 'og:site_name',
          content: `${entry['title']} - Talk by ${import.meta.env['NG_APP_NAME']}`,
        });
        this.metaTag.updateTag({
          name: 'og:title',
          content: `${entry['title']} - Talk by ${import.meta.env['NG_APP_NAME']}`,
        ...
        this.talk = entry;
        this.quote = this.selectRandomQuote(quote);
  ...
  private selectRandomQuote(payload: any) {
    if (payload?.quotes && Array.isArray(payload.quotes) && payload.quotes.length > 0) {
      const randomIndex = Math.floor(Math.random() * payload.quotes.length);
      return payload.quotes[randomIndex];
    }
